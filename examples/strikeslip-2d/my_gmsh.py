...
class App(GenerateMesh):
    ...
    def create_geometry(self):
        ...
        p5 = gmsh.model.geo.add_point(x1, -self.LAYER_THICKNESS, 0.0)
        p6 = gmsh.model.geo.add_point(x1+lx, -self.LAYER_THICKNESS, 0.0)

        # Create Trapezoid points.
        p7 = gmsh.model.geo.add_point(0.5 * x1, y1 + ly, 0.0)
        p8 = gmsh.model.geo.add_point(0.25 * x1, y1 + 0.75 * ly, 0.0)
        p9 = gmsh.model.geo.add_point(-0.25 * x1, y1 + 0.75 * ly, 0.0)
        p10 = gmsh.model.geo.add_point(-0.5 * x1, y1 + ly, 0.0)
        p11 = gmsh.model.geo.add_point(0.375 * x1, -self.LAYER_THICKNESS, 0.0)
        p12 = gmsh.model.geo.add_point(-0.375 * x1, -self.LAYER_THICKNESS, 0.0)
        ...
        self.c_bot = gmsh.model.geo.add_line(p1, p2)

        self.c_right_lower = gmsh.model.geo.add_line(p2, p6)
        self.c_right_layer = gmsh.model.geo.add_line(p6, p3)

        self.c_top_left = gmsh.model.geo.add_line(p7, p4)
        self.c_top_right = gmsh.model.geo.add_line(p3, p10)

        self.c_left_layer = gmsh.model.geo.add_line(p4, p5)
        self.c_left_lower = gmsh.model.geo.add_line(p5, p1)

        self.left_layer_bot = gmsh.model.geo.add_line(p11, p5)
        self.right_layer_bot = gmsh.model.geo.add_line(p12, p6)

        self.trap_top = gmsh.model.geo.add_line(p10, p7)

        self.trap_left_layer = gmsh.model.geo.add_line(p7, p11)
        self.trap_left_lower = gmsh.model.geo.add_line(p11, p8)

        self.trap_bot = gmsh.model.geo.add_line(p8, p9)

        self.trap_right_layer = gmsh.model.geo.add_line(p12, p10)
        self.trap_right_lower = gmsh.model.geo.add_line(p9, p12)
